...
def main():
    ...
    proj = post(
        "/projects/",
        json={"name": f"demo-azure-{int(time.time())}", "description": "demo deploy to azure"},
        headers={"content-type": "application/json"},
    ).json()
    pid = proj["id"]
    wi = post(
        "/work-items/",
        json={"project_id": pid, "title": "deploy:staging to azure", "description": "demo"},
        headers={"content-type": "application/json"},
    ).json()
    wi_id = wi["id"]
    ...
    print(
        ...
        "-d '{\"values\":{\"AZURE_CLIENT_ID\":\"...\",\"AZURE_TENANT_ID\":\"...\",\"AZURE_CLIENT_SECRET\":\"...\",\"AZURE_SUBSCRIPTION_ID\":\"...\"}}'"
    )
    print()
