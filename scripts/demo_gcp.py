...
def main():
    ...
    proj = post(
        "/projects/",
        json={"name": f"demo-gcp-{int(time.time())}", "description": "demo deploy to gcp"},
        headers={"content-type": "application/json"},
    ).json()
    pid = proj["id"]
    wi = post(
        "/work-items/",
        json={"project_id": pid, "title": "deploy:staging to gcp", "description": "demo"},
        headers={"content-type": "application/json"},
    ).json()
    wi_id = wi["id"]
    ...
    print(
        ...
        "-d '{\"values\":{\"GOOGLE_APPLICATION_CREDENTIALS_JSON\":\"{...service-account-json...}\",\"GOOGLE_CLOUD_PROJECT\":\"my-project\"}}'"
    )
    print()
